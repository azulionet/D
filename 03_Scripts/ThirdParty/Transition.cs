	...
	public class Transition : MonoBehaviour
	{
		...
		public static Transition Ins
		{
			get
			{
				if (_instance == null)
				{
					_instance = FindObjectOfType(typeof(Transition)) as Transition;

					if (_instance == null)
					{
						_instance = new GameObject("Transition", typeof(Transition)).GetComponent<Transition>();
					}
				}
		...
		public static void FadeAction(float duration, Color color, System.Action a_fpCallback)
		{
			var fade = new GameObject("Transition");
			fade.AddComponent<Transition>();
			fade.GetComponent<Transition>().StartFade(duration, color, a_fpCallback);
			fade.transform.SetParent(m_canvas.transform, false);
			fade.transform.SetAsLastSibling();
		}

		private void StartFade(float duration, Color fadeColor, System.Action a_fpCallback)
		{
			StartCoroutine(RunFadeAction(duration, fadeColor, a_fpCallback));
		}

		// This coroutine performs the core work of fading out of the current scene
		// and into the new scene.
		private IEnumerator RunFadeAction(float duration, Color fadeColor, System.Action a_fpCallback)
		{
			var bgTex = new Texture2D(1, 1);
			bgTex.SetPixel(0, 0, fadeColor);
			bgTex.Apply();
